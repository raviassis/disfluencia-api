...
module.exports = class BaseRepository {
  ...
  async deleteOne(id) {
    const conection = await this.dbClient.connect(this.uri, this.options);
    const result = (await conection.db().collection(this.collection).deleteOne({ _id: id })).ops[0];
    conection.close();
    return this._mapResultToModel(result);
  }
  ...
  async findMany(query) {
    const conection = await this.dbClient.connect(this.uri, this.options);
    const result = await conection.db().collection(this.collection).find({}).toArray();
    conection.close();
